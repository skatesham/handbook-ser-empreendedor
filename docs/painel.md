...
graph TB
    ...
    OBJ --> P1[🌱 Sustentabilidade]
    OBJ --> P2[🎨 Artesania Autoral]
    OBJ --> P3[💡 Criatividade Consciente]
    OBJ --> P4[🤝 Relações Íntegras]
    OBJ --> P5[⚙️ Excelência Operacional]
    
    P1 --> D1[Materiais responsáveis]
    P2 --> D2[Cada peça é única]
...
graph TB
    ...
    CEO --> PROD[🪵 Operações & Produção]
    CEO --> PRODUTO[🎨 Produto & Catálogo]
    CEO --> COM[🛒 Comercial]
    CEO --> FIN[💰 Financeiro]
    CEO --> PESS[❤️ Pessoas & Cultura]
    
    PROD --> EXEC[🔨 Execução]
    PRODUTO --> CAT[📷 Catalogação]
    COM --> VEND[💬 Vendas]
    FIN --> CONT[📋 Controle]
    PESS --> ROTI[🌱 Rotina]
